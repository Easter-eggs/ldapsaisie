...
class LSldapObject { 
  ...
  function updateData($idForm=NULL) {
    ...
    if($this -> validateAttrsData($idForm)) {
      LSdebug("les donn√©es sont valid√©es");
      ...
      if ($this -> submitChange($idForm)) {
        LSdebug('Les modifications sont submit√©es');
  ...
  /**
   * Methode crÈant la liste des objets en relations avec l'objet courant et qui
          ...
          if ($GLOBALS['LSsession'] -> loadLSobject($relation_conf['LSobject'])) {
            ...
            if ((method_exists($obj,$relation_conf['list_function']))&&(method_exists($obj,$relation_conf['getkeyvalue_function']))) {
              ...
              if (is_array($list)) {
                ...
                LSdebug('ProblËme durant la mise en cache de la relation '.$relation_name);
                return;
              }
            }
            else {
              LSdebug('Les mÈthodes de mise en cache de la relation '.$relation_name. ' ne sont pas toutes disponibles.');
              return;
            }
          }
          else {
  ...
  /**
   * Methode executant les actions nÈcÈssaires avant la suppression de
   * l'objet.
   * 
   * Cette mÈthode n'est qu'un exemple et elle doit Ítre certainement rÈÈcrite
   * pour les objets plus complexe.
  ...
  /**
   * Methode executant les actions nÈcÈssaires aprËs la suppression de
   * l'objet.
   * 
   * Cette mÈthode n'est qu'un exemple et elle doit Ítre certainement rÈÈcrite
   * pour les objets plus complexe.
  ...
  /**
   * Methode executant les actions nÈcÈssaires aprËs la crÈation de
   * l'objet.
   * 
   * Cette mÈthode n'est qu'un exemple et elle doit Ítre certainement rÈÈcrite
   * pour les objets plus complexe.
          ...
          if ($GLOBALS['LSsession'] -> loadLSobject($type)) {
            if (isset($GLOBALS['LSobjects'][$type]['container_auto_create'])&&isset($GLOBALS['LSobjects'][$type]['container_dn'])) {
              ...
              if(!$GLOBALS['LSldap'] -> getNewEntry($dn,$GLOBALS['LSobjects'][$type]['container_auto_create']['objectclass'],$GLOBALS['LSobjects'][$type]['container_auto_create']['attrs'],true)) {
                LSdebug("Impossible de crÈer l'entrÈe fille : ".print_r(
                  array(
                    'dn' => $dn,
  ...
  /**
   * Retourne la liste des relations pour l'objet en fonction de sa prÈsence 
   ...
   * est Ètablis par la prÈsence de la valeur de rÈfÈrence ‡ l'objet dans 
   ...
   * @param[in] $attr L'attribut dans lequel l'objet doit Ítre ajoutÈ
   * @param[in] $objectType Le type d'objet en relation
   * @param[in] $attrValue La valeur que doit avoir l'attribut :
   *                      - soit le dn (par defaut)
   *                      - soit la valeur [0] d'un attribut
   * 
   * @retval boolean true si l'objet ‡ ÈtÈ ajoutÈ, False sinon
   **/  
   ...
   * @param[in] $attr L'attribut dans lequel l'objet doit Ítre supprimÈ
   * @param[in] $objectType Le type d'objet en relation
   * @param[in] $attrValue La valeur que doit avoir l'attribut :
   *                      - soit le dn (par defaut)
   *                      - soit la valeur [0] d'un attribut
   * 
   * @retval boolean true si l'objet ‡ ÈtÈ supprimÈ, False sinon
   **/  
  ...
  * @param[in] $oldValue string L'ancienne valeur faisant rÈfÈrence ‡ l'objet
  * @param[in] $attr L'attribut dans lequel l'objet doit Ítre supprimÈ
  * @param[in] $objectType Le type d'objet en relation
  * @param[in] $attrValue La valeur que doit avoir l'attribut :
  *                      - soit le dn (par defaut)
  *                      - soit la valeur [0] d'un attribut
  *  
  * @retval boolean True en cas de succËs, False sinon
  */
   ...
   *                      - soit la valeur [0] d'un attribut
   * 
   * @retval boolean true si tout c'est bien passÈ, False sinon
   **/  
